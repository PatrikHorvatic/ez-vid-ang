...
export class EvaApi {
  ...
  public volumeChanged(_e: Event): void {
    ...
    this.videoVolumeSubject.next(this.assignedVideoElement!.volume);
  }
  ...
  private detectBuffering(currentTime: number): void {
    ...
    this.bufferingTimeout = setTimeout(() => {
      if (this.currentPlayPos === this.lastPlayPos && !this.assignedVideoElement!.paused && !this.assignedVideoElement!.ended && this.assignedVideoElement!.readyState < READY_STATE_HAVE_FUTURE_DATA) {
        this.isBuffering.set(true);
      }
