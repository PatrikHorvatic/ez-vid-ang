...
- **`EvaScrubBar` — new `evaThumbnailBaseUrl` input** (GitHub issue: no way to scope VTT thumbnail image requests to a non-fixed backend on self-hosted instances). Relative and root-relative (`/api/...`) thumbnail image URLs from the VTT file previously always resolved against the VTT file's own location or `window.location.origin`, with no way to override that for setups where the API serving thumbnail images is on a different origin than the page. When set, every non-absolute cue URL is resolved via `new URL(cueUrl, evaThumbnailBaseUrl)` instead; already-absolute URLs (`http://`, `https://`, `data:`) are unaffected. Changing the input at runtime re-fetches and re-resolves the currently loaded VTT.
- **`EvaScrubBar` — VTT thumbnail parsing failures are now logged**: `parseThumbnailVtt()` previously skipped invalid cue blocks with no diagnostic at all. It now emits a `console.warn` when every cue block in a successfully-fetched VTT fails to parse, or when some (but not all) cue blocks are invalid, naming the VTT URL and cue count. A missing VTT or failed/aborted fetch remains silent, as before — that's an expected "no thumbnails configured" state, not a parsing error.
...
- **`EvaOverlayPlay` left an uncovered strip of video when the controls bar auto-hid**: its height was hardcoded to `calc(100% - var(--eva-control-element-height))` regardless of whether `eva-controls-container` was actually visible, so once it auto-hid (or was never shown), a strip of video exactly the controls-bar's height stayed outside the overlay's click/background area. It now subscribes to `EvaApi.componentsContainerVisibilityStateSubject` (the same subject `EvaSubtitleDisplay` already uses for its own bottom-offset) and expands to `100%` while the controls bar is hidden, matching `EvaSubtitleDisplay`'s existing pattern.
- **`EvaDashDirective` — a custom `evaDashConfig` could silently re-enable auto-displayed manifest subtitles**: the directive's default settings (`streaming: { text: { defaultEnabled: false } }`) and the consumer's `evaDashConfig` were merged into a single object via a top-level spread before being passed to dash.js's `updateSettings()`. Since dash.js's settings tree is deeply nested, any `evaDashConfig` that set its own `streaming` key for an unrelated purpose (ABR tuning, buffer settings, live-delay, etc. — all extremely common) replaced the entire `streaming` object at the JS-object-literal level, wiping out `streaming.text.defaultEnabled` before dash.js ever saw it and silently reintroducing the `[22.0.8, 21.2.8]` auto-display bug. `updateSettings()` is now called twice — once with the directive's defaults, once with `evaDashConfig` — relying on dash.js's own settings tree deep-merging correctly across successive calls, so an unrelated `streaming` override no longer clobbers the subtitle-suppression default.
- **`EvaHlsDirective`/`EvaDashDirective` — switching the video source left the previous stream's quality list active**: `destroyPlayer()` cleared the registered audio and subtitle track lists on teardown but never called `EvaApi.registerQualityLevels([])`, unlike those other two. Changing `evaHlsSrc`/`evaDashSrc` to a new stream left `EvaQualitySelector` showing the *previous* stream's quality menu until (or unless) the new manifest parsed, and picking a level from it sent a stale/out-of-range index straight into the newly created player instance. `destroyPlayer()` now also clears the quality levels in both directives.
- **`EvaRemotePlayback` — the Safari AirPlay fallback leaked `<video>` element listeners**: `setupSafariFallback()` attached its two `webkit*` listeners as anonymous inline functions, and `teardown()` only ever removed listeners for the W3C Remote Playback API branch — there was no cleanup path for the Safari fallback at all, and the anonymous functions couldn't have been removed even if there were, since no reference to them was kept. Mounting/unmounting `<eva-remote-playback>` on Safari while reusing the same `<video>` element (conditional rendering, playlist reuse) accumulated duplicate listeners, each firing on every subsequent AirPlay event. The two handlers are now stored as bound instance methods and removed in `teardown()`, mirroring the existing W3C API cleanup.
- **`EvaTrackSelector` — an explicit subtitle selection could be silently reverted**: the dropdown's track list was fully rebuilt from `default` flags every time `EvaApi.videoTracksSubject` or `streamSubtitleTracksSubject` re-emitted, with no awareness of what was currently selected. Both subjects can legitimately re-emit mid-session (`evaVideoTracks` changing on a playlist swap, or hls.js re-firing `SUBTITLE_TRACKS_UPDATED` on a manifest reload for live streams), which silently reverted a user's explicit track choice — including an explicit "Off" — back to whatever the `default` flags computed. A new `hasExplicitSelection` flag, set only when the user actually picks a track via `selectTrack()`, now makes the rebuild preserve that choice (if the track is still present) instead of recomputing it from scratch; the neutral "Off" state present before any interaction is deliberately *not* sticky, so a `default: true` track can still auto-select normally the first time tracks actually arrive (e.g. tracks loading asynchronously after the player first renders).
- **`EvaScrubBar` — `hideWithControlsContainer` and `evaShowChapters` had no effect when changed at runtime**: both were only read once, in `ngOnInit()`. Toggling `hideWithControlsContainer` from `false` to `true` after init never started the controls-bar-synced auto-hide, and toggling `evaShowChapters` from `false` to `true` left `chapters()` permanently empty, since the `chapterMarkerChangesSubject` subscription was never established. `EvaScrubBar` now handles both reactively in `ngOnChanges` — mirroring `EvaControlsContainer`'s existing `evaAutohide` handling for the former, and lazily activating chapter tracking the first time `evaShowChapters` flips to `true` for the latter.
